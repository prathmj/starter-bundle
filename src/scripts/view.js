...
class View {
  ...
  getTeamId(sport) {
    ...
    const url = `http://api.stats.com/v1/stats/${this.sportUrlMap[sport]}/teams/?languageId=1&accept=jsonp&api_key=${key}&sig=${this.generateSignature(key, secret)}`
  ...
  getUrl(sport, team) {
    ...
    return `http://api.stats.com/v1/stats/${this.sportUrlMap[sport]}/teams/?accept=jsonp&api_key=${key}&sig=${this.generateSignature(key, secret)}`
  }

  getScheduleUrl(id, sport, standings) {
    let key = GLOBAL_VARS.statsApiCreds[sport].key;
    let secret = GLOBAL_VARS.statsApiCreds[sport].secret;
    ...
    if (standings) return `http://api.stats.com/v1/stats/${this.sportUrlMap[sport]}/standings/?accept=jsonp&api_key=${key}&sig=${this.generateSignature(key, secret)}`
    if (!id) return `http://api.stats.com/v1/stats/${this.sportUrlMap[sport]}/events/?accept=jsonp&api_key=${key}&sig=${this.generateSignature(key, secret)}`
    return `http://api.stats.com/v1/stats/${this.sportUrlMap[sport]}/scores/teams/${id}?date=${date}&accept=jsonp&api_key=${key}&sig=${this.generateSignature(key, secret)}`
  }

  renderLiveGame(event) {
    console.log('render LiveGame: ', event)
    this.iframe.src = './templates/liveGame/index.html';
  }

  renderFinalScore(event) {
    console.log('render FinalScore: ', event)
    this.iframe.src = './templates/finalScore/index.html';
  }

  renderCountdown(event) {
    console.log('render CountDown: ', event)
    this.iframe.src = './templates/countdown/index.html';
  }

  renderCurrentStandings() {
    const url = this.getScheduleUrl(null, this.sport, true)
    console.log(url)
    // this.jsonFetchPromise()
  }
        ...
        .then(res => {
          ...
          if (GLOBAL_VARS.variations.liveScore.active) {
            ...
            if (liveGame) {
              ...
              this.data = Object.assign(liveGame, {backgroundImage});
              this.renderLiveGame(liveGame);
              return;
            }
          }
          ...
          if (GLOBAL_VARS.variations.finalScore.active) {
            ...
            if (finalScore) {
              ...
              this.data = Object.assign(finalScore, {backgroundImage});
              this.renderFinalScore(finalScore);
              return;
            }
          }
          ...
          if (GLOBAL_VARS.variations.countdown.active) {
            ...
            if (countdown) {
              ...
              this.data = Object.assign(countdown, {backgroundImage});
